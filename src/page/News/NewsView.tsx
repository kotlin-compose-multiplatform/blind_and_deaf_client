            ...
            <Stack sx={{ pt: 4, pb: 4 }}>
              ...
              <Typography
                sx={{
                  ...
                  fontSize: isBlind ? "22px" : isMobile ? "12px" : "14px",
                }}
              >
                {convertTimeStampToDate(checkValue(data.updated_at))} /{" "}
                {t("views")}:{data.views}
              </Typography>
              ...
              <div
                dangerouslySetInnerHTML={{
                  __html: getLanguageValue(data, "content", appLanguage),
                }}
                style={{
                  maxWidth: "100%",
                  overflow: "hidden",
              ...
              <Spacer space={2} />
              {data.files ? (
                data.files?.filter((it) => it.mime_type == 0).length > 0 ? (
                  <VideoPlayer
                    src={getVideoPath(
                      data.files?.filter((it) => it.mime_type == 0)[0].url
                    )}
                    altImage={
                      data.files
                        ? getImagePath(
                            data.files?.filter((it) => it.mime_type == 1)[0].url
                          )
                        : ""
                    }
                    playIconStyle={
                      isMobile ? { width: "30px", height: "30px" } : undefined
                    }
                    style={{
                      width: "100%",
              ...
              <Stack spacing={2}>
                {data.files
                  ? data.files?.filter((it) => it.mime_type == 2).length > 0
                    ? data.files
                        ?.filter((it) => it.mime_type == 2)
                        .map((it) => {
